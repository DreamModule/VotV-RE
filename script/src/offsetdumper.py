...
*fields* = [(“lpBaseOfDll”, ctypes.c_void_p),
    ...
    if psapi.EnumProcessModules(h_process, modules_array, ctypes.sizeof(modules_array), ctypes.byref(cb_needed)):
        ...
        if count > 0:
            ...
            if module_handle:
                ...
                if psapi.GetModuleInformation(h_process, module_ptr, ctypes.byref(mod_info),
...
def is_valid_address(h_process, address):
if address is None or address == 0:
return False
test = read_memory(h_process, address, 8)
return test is not None and len(test) == 8
...
h_process = kernel32.OpenProcess(PROCESS_QUERY_INFORMATION | PROCESS_VM_READ | PROCESS_VM_OPERATION, False,
process_id)
if not h_process:
return None
